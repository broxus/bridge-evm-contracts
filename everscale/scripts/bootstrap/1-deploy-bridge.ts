...
const main = async (): Promise<void> => {
  ...
  const bridgeToken = await JettonMinter.deploy(
      admin.address,
      { name: "Bridge Test Token", symbol: "BRDG TEST", decimals: 9 },
      { value: toNano(1.5), bounce: true },
  );
  ...
  const { contract: bridge } = await locklift.factory.deployContract({
    ...
    constructorParams: {
      _owner: admin.address,
      _roundDeployer: staking.address,
