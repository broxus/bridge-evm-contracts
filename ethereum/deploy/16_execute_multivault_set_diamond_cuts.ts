...
const func: DeployFunction = async function ({ getNamedAccounts, deployments, ethers }: HardhatRuntimeEnvironment) {
    ...
    const settings = await ethers.getContract<MultiVaultFacetSettings>('MultiVaultFacetSettings');

    const {
        data: multiVaultInitialize
    } = await settings.initialize.populateTransaction(
        bridge_address,
        owner,
        weth
    );
...
export const tags = ['Deploy_MultiVault_Set_Diamond_Cuts'];
